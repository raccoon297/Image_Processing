...
본 프로젝트는 2인 팀으로 진행하였다.

### 윤동환

- I/Q 샘플의 AGC 정규화 및 2D 히스토그램 전처리 파이프라인을 설계하고 최적화하였다.
- MobileNetV3 Small을 성상도 기반 6클래스 AMC 모델로 개조하였다.
- 학습 및 검증 코드와 결과 시각화를 구성하였다.
- 프로젝트 결과 분석과 포트폴리오 자료 정리를 담당하였다.

### 박훈범

- 가상 변조 신호 생성 및 AWGN 노이즈 삽입 로직을 구현하였다.
- 가상 신호 추론 테스트 코드를 구성하였다.
- 과적합 제어 과정과 ResNet-18 비교 실험에 참여하였다.
